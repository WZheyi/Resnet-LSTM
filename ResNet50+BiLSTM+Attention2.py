...
class resnet_lstm(torch.nn.Module):
    ...
    def forward(self, x):  
        ...
        batch_size = x.size(1)
        h0 = torch.zeros(self.num_layers*2, B, self.hidden_size).to(device) # 同样考虑向前层和向后层
        c0 = torch.zeros(self.num_layers*2, B, self.hidden_size).to(device)
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each, save_result):
    ...
    for epoch in range(epochs):
        ...
        model_name = "phase_t200_v5_s5_e10_o1/lstm" \
        ...
        record_name = "phase_t200_v5_s5_e10_o1/lstm" \
                      + "_epoch_" + str(best_epoch) \
                      + "_length_" + str(sequence_length) \
                      + "_opt_" + str(optimizer_choice) \
                      + "_mulopt_" + str(multi_optim) \
...
def main():
    train_dataset, train_num_each, val_dataset, val_num_each, _, _ = get_data('/home/guest2/code/MTRCNet-CL-master/pytorch1.2.0/train40_val8_test32_paths_labels.pkl')
    csvfile = 'phase_t200_v5_s5_e10_o1/save_result.csv'
