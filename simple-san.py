...
train_batch_size = args.train  #100
val_batch_size = args.val  #100
...
class resnet_lstm(torch.nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, bidirectional = True)  # , bidirectional = True
        # hidden_size = 512
        # num_layers = 2 
        # num_classes = 7, 与手术阶段数对应
    ...
    def attention_net(self, lstm_output, final_state): 
        ...
        hidden = final_state.view(-1, hidden_size * 2, 2)   # hidden : [batch_size, n_hidden * num_directions(=2), 1(=n_layer)]
        attn_weights = torch.bmm(lstm_output, hidden).squeeze(2) # attn_weights : [batch_size, n_step]
        soft_attn_weights = F.softmax(attn_weights, 1)
        print("soft_attn_weights's shape is", soft_attn_weights.shape)
        print("attn_weights's shape is", attn_weights.shape)
        ...
        #soft_attn_weights.cpu()
        #Tensor.cpu()
        return context, soft_attn_weights.data.cpu().numpy() # context : [batch_size, n_hidden * num_directions(=2)]


    def forward(self, x):  # when train_batch = 20, value_batch = 20, time(sequence_length) = 4, the shape is 
        B, S, C, H, W = x.shape   #[5,4,3,224,224]
        print('='*50)
        print(x.shape) 
        
        x = x.view(-1, 3, 224, 224)   #[20,3,224,224]
        print('@'*50)
        print(x.shape)
        
        x = self.share.forward(x)  # [20,2048,1,1]
        print('#'*50)
        print(x.shape)
        
        x = x.view(-1, 2048)    
        x = x.view(-1, sequence_length, 2048)  #[5,4,2048]
        print('+'*50)
        print(x.shape)

        h_0 = Variable(torch.zeros(self.num_layers*2, B, self.hidden_size)).to(device) 
        c_0 = Variable(torch.zeros(self.num_layers*2, B, self.hidden_size)).to(device)

        self.lstm.flatten_parameters()
        
        out, (h_n, c_n) = self.lstm(x,(h_0,c_0))
        print('%'*50)
        print(out.shape)    #[5,4,1024]
        print(h_n.shape)    #[4,5,512]
        
        out = out.permute(1, 0, 2) 
        print('^'*50)
        print(out.shape)               #[4,5,1024]
        
        out = out.view(B, S, 1024)     
        h_n = h_n.view(B, 512, -1)
        print('>'*50)
        print(out.shape)               #[5,4,1024]
        print(h_n.shape)               #[5,512,4]

        y, attention = self.attention_net(out, h_n) # There is a problem here
        print('&'*50)

        y = y.contiguous().view(-1, 1024)
        print('*'*50)
        print(y.shape)
...
def get_useful_start_idx(sequence_length, list_each_length): 
    ...
    for i in range(len(list_each_length)):
        for j in range(count, count + (list_each_length[i] + 1 - sequence_length)): #(0,91) (100,291) 191
            idx.append(j)
        count += list_each_length[i] #100  0+100
    return idx #总共要计算的照片数
...
def train_model(train_dataset, train_num_each, val_dataset, val_num_each, save_result):
    with open(save_result, 'a') as f:
        line = 'epoch,train_elapsed_time,train_elapsed_time,train_average_loss,train_accuracy,val_elapsed_time ,val_elapsed_time , val_average_loss,val_accuracy \n'
        f.write(line)
    ...
    print(val_dataset)
    model = resnet_lstm(input_size, hidden_size, num_layers, num_classes)
    if use_gpu:
    ...
    for epoch in range(epochs):
        ...
        print('epoch: {:4d}'
              ...
              .format(epoch,
                      ...
                      val_accuracy))
        with open(save_result, 'a') as f:
            line = '{:4d} , {:2.0f}, {:2.0f} , {:4.4f},{:.4f},{:2.0f},{:2.0f},{:4.4f},{:.4f} \n'.format(
                      epoch,
                      train_elapsed_time // 60,
                      train_elapsed_time % 60,
                      train_average_loss,
        ...
        model_name = "phase_t1_v1_e2/self-attention" \
        ...
        record_name = "phase_t1_v1_e2/lstm" \
                      + "_epoch_" + str(best_epoch) \
                      + "_length_" + str(sequence_length) \
                      + "_opt_" + str(optimizer_choice) \
                      + "_mulopt_" + str(multi_optim) \
...
def main():
    train_dataset, train_num_each, val_dataset, val_num_each, _, _ = get_data('train60_val18_test2_paths_labels.pkl')
    csvfile = 'phase_t1_v1_e2/save_result.csv'
    train_model(train_dataset, train_num_each, val_dataset, val_num_each, csvfile)
